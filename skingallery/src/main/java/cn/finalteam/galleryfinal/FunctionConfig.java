...
public class FunctionConfig implements Cloneable, Parcelable {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeByte(this.mutiSelect ? (byte) 1 : (byte) 0);
        dest.writeInt(this.maxSize);
        dest.writeByte(this.editPhoto ? (byte) 1 : (byte) 0);
        dest.writeByte(this.crop ? (byte) 1 : (byte) 0);
        dest.writeByte(this.rotate ? (byte) 1 : (byte) 0);
        dest.writeByte(this.camera ? (byte) 1 : (byte) 0);
        dest.writeInt(this.cropWidth);
        dest.writeInt(this.cropHeight);
        dest.writeByte(this.cropSquare ? (byte) 1 : (byte) 0);
        dest.writeByte(this.rotateReplaceSource ? (byte) 1 : (byte) 0);
        dest.writeByte(this.cropReplaceSource ? (byte) 1 : (byte) 0);
        dest.writeByte(this.forceCrop ? (byte) 1 : (byte) 0);
        dest.writeByte(this.forceCropEdit ? (byte) 1 : (byte) 0);
        dest.writeByte(this.preview ? (byte) 1 : (byte) 0);
        dest.writeStringList(this.selectedList);
        dest.writeStringList(this.filterList);
    }

    protected FunctionConfig(Parcel in) {
        this.mutiSelect = in.readByte() != 0;
        this.maxSize = in.readInt();
        this.editPhoto = in.readByte() != 0;
        this.crop = in.readByte() != 0;
        this.rotate = in.readByte() != 0;
        this.camera = in.readByte() != 0;
        this.cropWidth = in.readInt();
        this.cropHeight = in.readInt();
        this.cropSquare = in.readByte() != 0;
        this.rotateReplaceSource = in.readByte() != 0;
        this.cropReplaceSource = in.readByte() != 0;
        this.forceCrop = in.readByte() != 0;
        this.forceCropEdit = in.readByte() != 0;
        this.preview = in.readByte() != 0;
        this.selectedList = in.createStringArrayList();
        this.filterList = in.createStringArrayList();
    }

    public static final Creator<FunctionConfig> CREATOR = new Creator<FunctionConfig>() {
        @Override
        public FunctionConfig createFromParcel(Parcel source) {
            return new FunctionConfig(source);
        }

        @Override
        public FunctionConfig[] newArray(int size) {
            return new FunctionConfig[size];
        }
    };
}
