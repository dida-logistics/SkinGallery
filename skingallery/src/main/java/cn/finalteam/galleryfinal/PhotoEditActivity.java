...
public class PhotoEditActivity extends CropImageActivity implements AdapterView.OnItemClickListener, View.OnClickListener {
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putSerializable("photoList", mPhotoList);
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        ArrayList<PhotoInfo> photoList = (ArrayList<PhotoInfo>) getIntent().getSerializableExtra("photoList");
        if (photoList != null) {
            mPhotoList = photoList;
        } else {
            mPhotoList = new ArrayList<>();
        }
        ArrayList<PhotoInfo> selectPhotoList = (ArrayList<PhotoInfo>) getIntent().getSerializableExtra("selectPhotoMap");
        if (selectPhotoList != null) {
            mSelectPhotoList = selectPhotoList;
        } else {
            mSelectPhotoList = new ArrayList<>();
        }
        mEditPhotoCacheFile = (File) savedInstanceState.getSerializable("editPhotoCacheFile");
        LinkedHashMap<Integer, PhotoTempModel> hashMap = (LinkedHashMap<Integer, PhotoTempModel>) getIntent()
                .getSerializableExtra("photoTempMap");
        mPhotoTempMap = new LinkedHashMap<>();
        if (hashMap != null && hashMap.size() > 0) {
            mPhotoTempMap.putAll(hashMap);
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (GalleryFinal.getFunctionConfig() == null || GalleryFinal.getGalleryTheme() == null) {
            ...
            mPhotoEditListAdapter = new PhotoEditListAdapter(this, mPhotoList, getScreenWidth());
            ...
            initCrop(mIvCropPhoto, GalleryFinal.getFunctionConfig().isCropSquare(), GalleryFinal.getFunctionConfig()
                    .getCropWidth(), GalleryFinal.getFunctionConfig().getCropHeight());
    ...
    protected void takeResult(PhotoInfo info) {
        ...
        if (!GalleryFinal.getFunctionConfig().isEditPhoto() && mTakePhotoAction) {
            ...
            PhotoSelectActivity activity = (PhotoSelectActivity) ActivityManager.getActivityManager().getActivity
                    (PhotoSelectActivity.class.getName());
            if (activity != null) {
    ...
    private void loadImage(PhotoInfo photo) {
        ...
        GalleryFinal.getCoreConfig().getImageLoader().displayImage(this, path, mIvSourcePhoto, mDefaultDrawable, getScreenWidth
                (), getScreenHeight());
    }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.fab_crop) {
            ...
            if (mPhotoList.size() > 0) {
                PhotoInfo photoInfo = mPhotoList.get(mSelectIndex);
                String ext = FilenameUtils.getExtension(photoInfo.getPhotoPath());
                if (StringUtils.isEmpty(ext) || !(ext.equalsIgnoreCase("png") || ext.equalsIgnoreCase("jpg") || ext
            ...
            if (GalleryFinal.getFunctionConfig().isMutiSelect() && GalleryFinal.getFunctionConfig().getMaxSize() ==
            ...
            if (mCropState && !(mCropPhotoAction && !GalleryFinal.getFunctionConfig().isRotate() && !GalleryFinal
                    .getFunctionConfig().isCamera())) {
                if ((GalleryFinal.getFunctionConfig().isForceCrop() && GalleryFinal.getFunctionConfig().isForceCropEdit())) {
                    mIvCrop.performClick();
                    return;
                }
    ...
    private void rotatePhoto() {
        if (null != mPhotoList && mPhotoList.size() > 0 && mPhotoList.get(mSelectIndex) != null && !mRotating) {
            String extTemp = null;
            final PhotoInfo photoInfo = mPhotoList.get(mSelectIndex);
            if (null != photoInfo) {
                extTemp = FilenameUtils.getExtension(photoInfo.getPhotoPath());
            }
            if (StringUtils.isEmpty(extTemp) || !(extTemp.equalsIgnoreCase("png") || extTemp.equalsIgnoreCase("jpg") || extTemp
                    .equalsIgnoreCase("jpeg"))) {
                toast(getString(R.string.edit_letoff_photo_format));
                return;
            }
            ...
            if (photoInfo != null) {
                ...
                new AsyncTask<Void, Void, Bitmap>() {
                    ...
                    protected void onPreExecute() {
                        super.onPreExecute();
                        mTvEmptyView.setVisibility(View.VISIBLE);
                        mProgressDialog = ProgressDialog.show(PhotoEditActivity.this, "", getString(R.string.waiting), true,
                                false);
                    }
                    ...
                    protected Bitmap doInBackground(Void... params) {
                        ...
                        Bitmap bitmap = Utils.rotateBitmap(path, orientation, getScreenWidth(), getScreenHeight());
                        if (bitmap != null) {
